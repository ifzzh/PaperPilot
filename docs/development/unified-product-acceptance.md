...
本轮交付以 1.1.3 为目标。1.1.0 保留为未部署候选；1.1.1 的稳定阅读位置验收失败后已回退。历史测试通过不替代最终镜像及正式服务的实际验收。
...
完整 Docker 非集成、安全扫描、SBOM、远端 CI 和固定摘要运行检查是额外发布门禁，不能代替上述产品体验。正式论文截图、文本、文件指纹、账号会话及受限备份路径只保存在私有开发证据中；公开 Release 附脱敏报告。
