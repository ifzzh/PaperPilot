    ...
    def __init__(
        ...
        *,
        ...
        self.base_url = (
            base_url
            or brand_getenv(
                "IPAPER_TRANSLATION_WORKER_URL",
                "http://ipaper-translation-worker:7192",
            )
        ).rstrip("/")
        self.token_file = Path(
            token_file
            or brand_getenv(
                "IPAPER_WORKER_TOKEN_FILE",
                "/run/secrets/ipaper_worker_token",
            )
        )
        self.jobs_root = Path(
            jobs_root
            or brand_getenv("IPAPER_TRANSLATION_JOBS_ROOT", "/work/jobs")
        )
