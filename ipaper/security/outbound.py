...
from ipaper.security.identity import current_identity
...
class OutboundPolicy:
    ...
    def from_environ(cls, environ: Mapping[str, str]) -> "OutboundPolicy":
        return cls(
            public_origins=parse_origin_list(brand_getenv("IPAPER_AI_ALLOWED_ORIGINS", environ=environ)),
            private_origins=parse_origin_list(
                brand_getenv("IPAPER_AI_PRIVATE_ALLOWED_ORIGINS", environ=environ), private=True
            ),
            transfer_origins=parse_origin_list(
                brand_getenv("IPAPER_MINERU_TRANSFER_ALLOWED_ORIGINS", environ=environ)
            ),
            proxy_fake_ip_networks=parse_proxy_fake_ip_networks(
                brand_getenv("IPAPER_AI_PROXY_FAKE_IP_RANGES", environ=environ)
            ),
        )
