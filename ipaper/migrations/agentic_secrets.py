...
def _backup_database(db_path: Path, backup_dir: Path) -> Path:
    ...
    backup_path = backup_dir / f"ipaper-pre-agentic-secrets-{timestamp}.db"
...
def rollback_migration(manifest_file: str | os.PathLike[str]) -> Path:
    ...
    _assert_exclusive_access(target)
    descriptor, temporary_name = tempfile.mkstemp(prefix=".ipaper-rollback-", dir=target.parent)
    os.close(descriptor)
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Manage encrypted iPaper agentic secrets")
    parser.add_argument("--db", default="db/ipaper.db")
