...
5. **Run the Application**
   ...
   **Custom Launch Arguments:**
   ...
   Production containers use one Gunicorn `gthread` worker with eight threads. The maintained Compose file uses the v0.10.2 Web, translation-worker, and document-worker images. The Web service binds only `127.0.0.1:7191`; Worker ports `7192` and `7193` are internal only. All three run as non-root users.
...
Stop all services first, back up SQLite and the paper directory, then run `python -m paperpilot.migrations.v010_workflows --dry-run` followed by `--apply` as documented in [the v0.10.2 release notes](docs/releases/v0.10.2.md). The migration is additive and does not move PDFs. v0.10.0 has a legacy-schema migration ordering defect; v0.10.2 also preserves database and settings permissions during rollback. Deploy all three v0.10.2 image digests together. Rollback restores the migration manifest backup and all three v0.9.1 image digests.
